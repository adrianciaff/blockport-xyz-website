...
interface GlobeAnimationProps {
  ...
  disableZoom?: boolean;
  disablePan?: boolean;
  enableBeamAnimation?: boolean; // <<<< NEW PROP to enable/disable beams
  className?: string;
}
...
const GlobeAnimation: React.FC<GlobeAnimationProps> = (props) => {
  ...
  useEffect(() => {
    ...
    controls.enableDamping = true; controls.dampingFactor = 0.05; controls.minDistance = 0.4; controls.maxDistance = 10; controls.target.set(0, 0, 0); controls.enablePan = !config.disablePan; controls.enableZoom = !config.disableZoom;
    ...
    let citiesData: any[] = [];
    const baseMarkersGroup = new THREE.Group(); globeGroup.add(baseMarkersGroup); // Group always added
    const beamsGroup = new THREE.Group(); globeGroup.add(beamsGroup); // Group always added
    ...
    const beamGradientTexture = createGradientTexture(); // Texture created even if not used (minor inefficiency)
    ...
    if (config.enableBeamAnimation) { // <<<< WRAPPER IF STATEMENT
        ...
        fetch('/worldcities-validator.csv')
            ...
            .then(csvText => {
               Papa.parse(csvText, { header: true, skipEmptyLines: true, dynamicTyping: true,
                   complete: (results) => {
                       if (results.errors.length) { console.error('GlobeAnimation: CSV Errors:', results.errors); return; } // Return on error
                       let rawParsedData = results.data || []; citiesData = rawParsedData;
                       console.log(`GlobeAnimation: CSV parsed: ${citiesData.length} cities.`);
                       // Create BASE Markers
                       ...
                       console.log(`GlobeAnimation: Added ${markersAddedCount} base markers. Aligned length: ${citiesData.length}`);
        ...
        console.log("GlobeAnimation: Beam animation disabled by prop."); // Log if disabled
    }
  ...
  }, []); // Keep empty dependency array for run-once on mount
