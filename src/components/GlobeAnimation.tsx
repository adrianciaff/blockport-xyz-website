...
const GlobeAnimation: React.FC<GlobeAnimationProps> = (props) => {
  ...
  const tweenGroup = useRef(new Group()); // Use ref for tween group
  ...
  useEffect(() => {
    ...
    camera.position.z = config.initialCameraZ;
    const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true });
    renderer.setSize(currentMountRef.clientWidth, currentMountRef.clientHeight);
    currentMountRef.appendChild(renderer.domElement);

    const controls = new OrbitControls(camera, renderer.domElement);
    controls.enableDamping = true; controls.dampingFactor = 0.05; controls.minDistance = config.minDistance; controls.maxDistance = config.maxDistance; controls.target.set(0, 0, 0); controls.enablePan = !config.disablePan; controls.enableZoom = !config.disableZoom;
    ...
    fetch('/ne_110m_land.geojson').then(r => r.ok ? r.json() : Promise.reject(`HTTP ${r.status}`)).then(geojsonData => {
        const linesGroup = new THREE.Group(); /* ... process geojson same as before ... */
    ...
    const pulseDuration = 200; const pulsePeakScale = config.pulsePeakScale; // Use prop
    ...
    if (config.enableBeamAnimation) {
        ...
        fetch('/worldcities-validator.csv').then(/* ... */).then(csvText => {
            Papa.parse(csvText, { /* ... */
                complete: (results) => { /* ... */ citiesData = results.data || []; /* ... */
                    // Create Base Markers
                    let markersAddedCount = 0;
                    citiesData.forEach((city: any, index: number) => { /* ... create markers ... */
                    ...
                    citiesData = baseMarkersGroup.children.map((marker: any)=>({...marker.userData})); console.log(`GlobeAnimation: Added ${markersAddedCount} base markers.`);
                    while(beamsGroup.children.length>0){ beamsGroup.remove(beamsGroup.children[0]); } currentBeam.mesh=null;
                    // Start Timer
                    if (citiesData.length > 0) { console.log('GlobeAnimation: Starting highlighting timer...'); const firstIndex = weightedRandomSelect(citiesData); if (firstIndex !== -1) createAndStartBeamAnimation(firstIndex); intervalId.current = setInterval(() => { const index=weightedRandomSelect(citiesData); if (index!==-1)createAndStartBeamAnimation(index); }, currentBeam.duration); } else { console.warn("GlobeAnimation: No valid city data, timer not started."); } },
    ...
    // === Animation Loop (Corrected Cleanup) ===
    const animate = (time: number) => {
      ...
      if (config.enableBeamAnimation && currentBeam.mesh) {
         ...
         if(el>=currentBeam.duration){ beamsGroup.remove(currentBeam.mesh); currentBeam.mesh=null; }
         else { const sn=Math.floor(el/currentBeam.slotDuration);
             if(sn>currentBeam.lastAnimatedSlot){
                 currentBeam.lastAnimatedSlot=sn; // Mark slot as animated
                 // Removed the problematic forEach loop
                 currentBeam.mesh.scale.set(0.01,1,0.01); // Reset scale
                 new Tween(currentBeam.mesh.scale, tweenGroup.current).to({ x:config.pulsePeakScale, y:1, z:config.pulsePeakScale }, pulseDuration / 2).easing(Easing.Quadratic.Out)
      ...
      tweenGroup.current.update(time); // Use group ref
      controls.update();
